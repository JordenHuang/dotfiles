...
vim.keymap.set("n", "<A-C-j>", ":m +1<CR>==", opts("Move text up"))
vim.keymap.set("n", "<A-C-k>", ":m -2<CR>==", opts("Move text down"))

-- Shift visual selected lines up and down
vim.keymap.set("v", "<A-C-j>", ":m '>+1<CR>gv=gv")
vim.keymap.set("v", "<A-C-k>", ":m '<-2<CR>gv=gv")

-- Window easy navigation
vim.keymap.set("n", "<A-h>", "<C-w>h", opts("Nevigate to left(h) window"))
vim.keymap.set("n", "<A-j>", "<C-w>j", opts("Nevigate to lower(j) window"))
vim.keymap.set("n", "<A-k>", "<C-w>k", opts("Nevigate to upper(k) window"))
vim.keymap.set("n", "<A-l>", "<C-w>l", opts("Nevigate to right(l) window"))
...
vim.cmd('cnoremap <expr> <C-A> pumvisible() ? "\\<C-a>" : "\\<Home>"')
vim.cmd('cnoremap <expr> <C-E> pumvisible() ? "\\<C-e>" : "\\<End>"')
