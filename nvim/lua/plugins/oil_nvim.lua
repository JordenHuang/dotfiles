-- TODO: Make this function a util in my_scripts
local function change_root_to_last_buffer_dir()
    ...
    for _, buf_nr in ipairs(vim.api.nvim_list_bufs()) do
        ...
        if lastused_buf_time_stamp < cur_time_stamp and buf_nr ~= oil_buf then
    ...
    oil.open(last_buf_dir)
end

local function oil_open_dir()
    return vim.fn.feedkeys(":Oil "..vim.fn.expand("%:p:h").."/")
end
...
local oil = {
    ...
    config = function()
        local function opts(description, buf_local)
            return { desc = description, noremap = true, silent = true }
        end

        -- open file_browser with the path of the current buffer
        vim.keymap.set("n", "<leader>oo", ":Oil<CR>", opts("Open oil file browser"))
        vim.keymap.set("n", "<leader>oO", oil_open_dir, opts("Open oil file browser, with path"))
        vim.keymap.set("n", "<leader>ob", change_root_to_last_buffer_dir, opts("Open oil file browser in last buffer's path"))
